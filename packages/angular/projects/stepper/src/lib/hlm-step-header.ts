...
@Component({
  ...
  host: {
    class:
      'tw:group tw:inline-flex tw:shrink-0 tw:outline-none tw:items-center tw:gap-2 tw:touch-manipulation tw:transition-opacity tw:data-[disabled=true]:pointer-events-none tw:data-[disabled=true]:opacity-50',
...
export class HlmStepHeader extends CdkStepHeader {
  ...
  constructor() {
    super();
    classes(() => [this.labelPosition() === 'bottom' && 'tw:flex-col tw:text-center']);
  }
  ...
  protected readonly _labelClass = computed(() =>
    hlm(
      'tw:flex tw:min-w-0 tw:touch-manipulation tw:flex-col tw:truncate tw:text-sm tw:font-medium',
      this.state() === 'error' && 'tw:text-destructive',
    ),
  );
