...
export class HlmStepper extends CdkStepper {
  ...
  protected _gridContainerClass(expanded: boolean): string {
    return hlm(
      ...
      this.animationsEnabled() && 'transition-[grid-template-rows]',
    );
  }

  protected _connectorClass(index: number): string {
    return hlm(
      'block h-full w-px transition-colors duration-200 motion-reduce:transition-none',
      index < this.selectedIndex ? 'bg-primary' : 'bg-border',
    );
  }
